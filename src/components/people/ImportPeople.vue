<template>
  <div class="w-full container mx-auto" style="min-width: 500px">
    <div class="w-full control ml-1 mt-10">

      <div class="w-full">
        <div class="w-full flex-col items-center mt-4">
          <div class="w-full control mr-1">
            <div>
              <label class="block text-grey text-sm font-bold mb-2">
                <span class="truncate">Choose File To Import From: ({{accepted.join(', ')}})</span>
              </label>
              <div class="h-32 relative" :class="getInputColor('People File')">
                <input type="file" id="People File" placeholder="Upload People File" class="text-grey-darker is-custom"
                  name="People File" tabindex="1" @change="fileUploaded" v-validate="{required: true, ext: accepted}"
                  ref="file-input">
          ...
          <div class="w-full control" v-if="file">
            <div class="flex items-center justify-center">
              <p class="text-center bg-grey-light text-grey-darker p-2 rounded inline-block">
                {{file.name}}
              </p>
            </div>
          </div>
      ...
      <div class="mt-8  inline-flex justify-start w-full">
        <div class="w-full inline-flex justify-between items-center">
          <button class="inline-flex flex-auto items-center justify-center h-10 bg-blue-light rounded-sm text-white shadow-md mr-2 p-6"
            @click="importPeople" v-show="file">
            ...
            <span class="font-bold uppercase">IMPORT</span>
            ...
            xml:space="preserve" v-if="loading">
            <g>
