<template>
  <div class="w-full flex flex-wrap lg:min-h-screen bg-blue-lightest-more font-serif" id="people">
            ...
            <div class="flex items-baseline justify-between">
              ...
              <on-click-outside :do="() => orderMenu = false" :active="orderMenu">
                <div class="inline-flex items-center justify-center cursor-pointer relative mr-4" @click="orderMenu = !orderMenu">
                  <div class="self-end">
                    <p class="text-grey-dark text-xs font-hairline">Sort by:</p>
                  </div>
                  <div class="inline-flex items-center">
                    <p class="text-xs ml-1 text-grey font-bold">{{order.text}}</p>
                  ...
                  <div v-show="orderMenu" class="z-10 mt-px text-sm text-center shadow-md text-grey-darker leading-normal rounded bg-white border absolute animated zoomIn flex flex-col overflow-hidden"
                    ...
                    <a v-for="order in peopleOrders" :key="order.id" class="cursor-pointer no-underline flex items-center justify-start px-4 py-3 whitespace-no-wrap group hover:text-white hover:bg-blue-light"
              ...
              <on-click-outside :do="() => sortMenu = false" :active="sortMenu">
                <div class="inline-flex items-center justify-center cursor-pointer relative" @click="sortMenu = !sortMenu">
                  <div class="self-end">
                    <p class="text-grey-dark text-xs font-hairline">Order:</p>
                  </div>
                  <div class="inline-flex items-center">
                    <p class="text-xs ml-1 text-grey font-bold">{{sort.text}}</p>
                    <svg class="h-2 w-2 fill-current text-grey-dark ml-1 self-center" version="1.1" xmlns="http://www.w3.org/2000/svg"
                      xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="292.362px" height="292.362px"
                      viewBox="0 0 292.362 292.362" style="enable-background:new 0 0 292.362 292.362;" xml:space="preserve">
                      <g>
                  ...
                  <div v-show="sortMenu" class="z-10 mt-px text-sm text-center shadow-md text-grey-darker leading-normal rounded bg-white border absolute animated zoomIn flex flex-col overflow-hidden"
                    style="min-width: 200px; top: 100%;">
                    <a v-for="sort in peopleSorts" :key="sort.id" class="cursor-pointer no-underline flex items-center justify-start px-4 py-3 whitespace-no-wrap group hover:text-white hover:bg-blue-light"
              ...
              <div class="inline-flex">
                ...
                <!-- <on-click-outside :do="() => viewMenu = false" :active="viewMenu">
                  <div class="inline-flex items-center justify-center cursor-pointer relative">
                    <button
                      class="bg-white text-grey-dark mr-4 h-8 w-8 rounded hover:bg-grey-light hover:text-white"
                    ...
                    <div
                      v-show="viewMenu"
                      class="z-10 mt-px text-sm text-center shadow-md text-grey-darker leading-normal rounded bg-white border absolute animated zoomIn flex flex-col overflow-hidden"
                      style="min-width: 200px; right: 25%; top: 100%;"
                    >
                      <a
                        v-for="view in PeopleViews"
                        :key="view.id"
                        class="cursor-pointer no-underline flex items-center justify-start px-4 py-3 whitespace-no-wrap group hover:text-white hover:bg-blue-light"
          ...
          <div class="container m-auto h-full relative">
            <div class="absolute" style="top: 20%; left: 50%; transform: translate(-50%, -50%);">
              ...
              <h4 class="text-grey text-xl text-center mt-4">
                Welcome Here! You have not added any members to your Church.
                Please use the button above to add people.
              </h4>
            </div>
          </div>
        </div>
      ...
      <template v-else>
        ...
        <div class="mt-6 py-1">
          <div class="w-full inline-flex justify-between justify-center">
            <p class="text-grey">Bulk Actions</p>
            <p class>
              <span class="text-grey-darker mr-1">{{selectedPeople.length}}</span>
    ...
    <transition enter-active-class="animated slideInRight" leave-active-class="animated slideOutRight">
      <div class="bg-white p-6 fixed pin-t pin-r h-screen overflow-y-auto shadow-lg z-20" :class="`${sideBarWith}`"
        v-show="takingAction" style="animation-duration: 500ms;">
        ...
        <div>
          <div class="absolute flex items-center text-white text-sm font-bold px-4 py-3 w-full mb-4 animated tada" role="alert"
            ...
            <svg class="h-4 w-4 fill-current text-white mr-4" version="1.1" xmlns="http://www.w3.org/2000/svg"
              ...
              <g>
                <path d="M479.963,425.047L269.051,29.854c-5.259-9.88-15.565-16.081-26.782-16.081h-0.03c-11.217,0-21.492,6.171-26.782,16.051L3.603,425.016c-5.046,9.485-4.773,20.854,0.699,29.974c5.502,9.15,15.413,14.774,26.083,14.774H453.12c10.701,0,20.58-5.594,26.083-14.774C484.705,445.84,484.979,434.471,479.963,425.047z M242.239,408.965c-16.781,0-30.399-13.619-30.399-30.399c0-16.78,13.619-30.399,30.399-30.399c16.75,0,30.399,13.619,30.399,30.399C272.638,395.346,259.02,408.965,242.239,408.965zM272.669,287.854c0,16.811-13.649,30.399-30.399,30.399c-16.781,0-30.399-13.589-30.399-30.399V166.256c0-16.781,13.619-30.399,30.399-30.399c16.75,0,30.399,13.619,30.399,30.399V287.854z"></path>
              </g>
            </svg>
          ...
          <view-person v-if="activeAction === 'view'" :person="editingPerson" @close="hideActionDrawer"></view-person>
        </div>
      </div>
    </transition>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      showActions() {
        console.log(this.selectedPeople.length > 0, 'actions')
    ...
    methods: {
      ...
      async doPeopleSearch(name, page = 1, size = 25) {
        if (!name) {
          this.refresh()
          return
        }
        ...
        const path =
          `filter_search_people/${search}?page=${page}&size=${size}&order=${
            this.order.value
      ...
      setUpPeopleUI() {
        if (!this.order) {
          ...
          this.order =
            this.peopleOrders.find(o => o.value === order) ||
            this.peopleOrders[0]
        }
        ...
        if (!this.sort) {
          ...
          this.sort =
            this.peopleSorts.find(s => s.value === sort) || this.peopleSorts[0]
        }

        if (!this.view) {
          const view = localStorage.getItem('people-view')
          this.view =
            this.PeopleViews.find(v => v.value === view) || this.PeopleViews[0]
        }
      },
      ...
      async getPeople(page = 1, size = 25) {
        ...
        if (!filters) {
          filters = this.$refs['filters'] && this.$refs['filters'].getActiveFilters()
        }!filters && console.log('filter absent', this.$refs['filters'])
        if (!filters) return

        const path =
          `get_people_with_filter?page=${page}&size=${size}&order=${
            this.order.value
          }&sort=${this.sort.value}`
        this.loadingMore = true
